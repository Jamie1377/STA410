...
class GradientDescentRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_iter=1000, lr=0.01, alpha=0.0001, momentum=0.9, batch_size=None):
        self.n_iter = n_iter
        self.lr = lr
    ...
    def _fit_gd(self, X, y):
        ...
        for _ in range(self.n_iter):
            ...
            gradients += self.alpha * self.coef_  # L2 regularization
    ...
    def _fit_sgd(self, X, y):
        ...
        for _ in range(self.n_iter):
            ...
            gradients = 2/self.batch_size * X_batch.T @ (X_batch @ self.coef_ - y_batch)
            gradients += self.alpha * self.coef_
            ...
            # Store loss
            loss = np.mean((X_batch @ self.coef_ - y_batch)**2) + 0.5*self.alpha*np.sum(self.coef_**2)
            self.loss_history.append(loss)
...
class ARIMAXGBoost(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, xgb_params=None):
        ...
        self.gd_model = GradientDescentRegressor(n_iter=2000, lr=0.1, alpha=0.01)
